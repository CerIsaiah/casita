...
**To just look at it, use the [live demo](https://casita-henna.vercel.app).** That
is the real thing, built from a real scrape, and it needs nothing from you.

The original fixture-backed demo still runs credentials-free:
...
Building my page locally is a different story, and worth being straight about:
the scraped data set is not in this repo. Roughly 2,600 listings of third-party
content and a set of owner names read out of public records do not belong in a
public tree, so `app_data.json`, the routing graph and the built page are all
gitignored. Rebuilding it means running the pipeline yourself:

```bash
cd research
cp ../.env.example ../.env   # add APIFY_API_TOKEN for Apartments.com
./refresh.sh                 # full sweep, ~1 hour
python3 pipeline/build_pages.py --new
python3 serve.py             # http://127.0.0.1:8799/casita-demo.html
```

Owner lookups additionally need `RENTCAST_API_KEY` and are capped at 50 a month
in code. Everything paid is opt-in; nothing here runs without a key you supplied
on purpose.
